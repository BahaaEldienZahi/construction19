    ...
    def _check_boq_line_project_match(self):
        for line in self:
            ...
            if line.boq_line_id and line.boq_line_id.project_id != project:
                raise ValidationError(_('Existing BOQ line does not belong to the same project as the variation order.'))
            if line.parent_boq_line_id and line.parent_boq_line_id.project_id != project:
                raise ValidationError(_('Parent BOQ line does not belong to the same project as the variation order.'))
